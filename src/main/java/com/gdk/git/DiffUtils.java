...
public class DiffUtils {
	...
	public static DiffOutput getDiff(Repository repository, RevCommit baseCommit, RevCommit commit, String path,
		...
		try {
			...
			switch (outputType) {
			case HTML:
				df = new GitDiffFormatter(commit.getName(), repository, path, handler, tabLength);
				break;
			case PLAIN:
			default:
			...
			df.flush();
			if (df instanceof GitDiffFormatter) {
				// workaround for complex private methods in DiffFormatter
				diff = ((GitDiffFormatter) df).getHtml();
				stat = ((GitDiffFormatter) df).getDiffStat();
			} else {
				diff = os.toString();
			}
		} catch (Throwable t) {
	 ...
	 *
	 * @param repository
	 * @param base if base commit is unspecified, the diffstat is generated against
	 *            the primary parent of the specified tip.
	 * @param tip
	 * @param path if path is specified, the diffstat is generated only for the
	 ...
	 *
	 * @param repository
	 * @param baseCommit if base commit is unspecified, the diffstat is generated against
	 *            the primary parent of the specified commit.
	 * @param commit
	 * @param path if path is specified, the diffstat is generated only for the
	 *            specified file or folder. if unspecified, the diffstat is
